...
1. note_index import 行改为：

```python
from ..note_index import NoteMeta, get_note_index
```

1. 紧跟 `semantic_orphan_indices` 函数定义之后，新增：
...
- [x] **Step 1: Update the three固化断言 + 降级断言（failing tests）**
...
1. `test_diagnose_all_orphan_vectors_returns_empty_clusters` 中（当前约 L311），把
...
1. `test_diagnose_dense_limit_counts_only_verified_unique_live_rows` 中（当前约 L542），把
...
1. `test_diagnose_filesystem_failure_skips_unverified_semantic_clusters` 的断言区末尾（`assert any("semantic clustering was skipped" ...)` 之后）补：
...
- [x] **Step 2: Write two new failing tests**
...
1. `diagnose_moc_density` 的 meta 构造段（当前约 L233-244），把
...
1. 赋值段（当前约 L292 `coverage.vector_orphans = orphan_count`）之后加一行：
...
1. `test_diagnose_json_contract` 中，`assert payload["coverage"]["vector_orphans"] == 2` 之后加：

```python
    assert payload["coverage"]["archived_in_index"] == 2
```

1. 新增两个测试（放在 `test_diagnose_table_has_four_sections_and_permanent_only_coverage` 之后）：
...
1. `_render_table`（当前约 L200-201），在 `_console.print(coverage_table)` 之后、`for warning in report.coverage.warnings:` 之前插入：
