...
def test_knowledge_graph():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        config = ZKConfig(base_dir=Path(tmpdir))
        config.ensure_dirs()
        ...
        assert note1.id in orphans, "note1 should be an orphan (no incoming links)"
        assert note2.id not in orphans, "note2 should not be an orphan"
...
def test_note_manager():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        found_path = NoteManager.find_note_file(config, note1.id)
...
def test_daily_inbox_commands():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 模拟 inbox 命令逻辑
        fleeting_notes = note_module.list_notes(note_type=NoteType.FLEETING, limit=20)
