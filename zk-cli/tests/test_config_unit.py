...
class TestGetDefaultKBPath:
    ...
    def test_uses_global_config(self):
        """测试使用全局配置获取路径"""
        with patch('zk.global_config.get_global_config_manager') as mock_get:
            mock_manager = Mock()
...
class TestZKConfig:
    ...
    def test_default_values(self):
        """测试默认值"""
        with patch('zk.config.get_default_kb_path', return_value=Path("/default")):
            config = ZKConfig()
    ...
    def test_load_from_default_path(self, tmp_path):
        ...
        with patch('zk.config.get_default_kb_path', return_value=tmp_path):
...
class TestGetConfig:
    ...
    def test_returns_zkconfig_instance(self):
        """测试返回 ZKConfig 实例"""
        with patch('zk.config.ZKConfig.load') as mock_load:
...
class TestUseKB:
    ...
    def test_use_kb_switches_config(self):
        """测试 use_kb 切换配置"""
        from zk.config import config
        
        original_dir = config.base_dir
        
        with patch('zk.kb_manager.get_kb_manager') as mock_get_manager:
            mock_manager = Mock()
    ...
    def test_use_kb_resets_search_indices(self):
        ...
        with patch('zk.kb_manager.get_kb_manager') as mock_get_manager:
            ...
            with patch('zk.bm25_index.reset_bm25_index') as mock_reset_bm25:
                with patch('zk.search_engine.reset_search_engine') as mock_reset_search:
                    with use_kb("test_kb"):
                        pass
    ...
    def test_use_kb_handles_reset_errors(self):
        ...
        with patch('zk.kb_manager.get_kb_manager') as mock_get_manager:
            ...
            with patch('zk.bm25_index.reset_bm25_index', side_effect=Exception("Reset error")):
                # 应该不抛出异常
                with use_kb("test_kb"):
                    pass
