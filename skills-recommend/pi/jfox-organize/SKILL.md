...
description: |
  ...
  Use when user wants to organize their knowledge base, process inbox,
  refine fleeting notes into permanent notes, add wiki links, merge notes,
  or optimize the knowledge graph.
  Triggers on: "整理知识库", "清理 inbox", "提炼笔记", "组织笔记",
  "看看有什么可以整理的", "合并笔记", "生成链接", "organize",
  "process inbox", "refine notes", "knowledge graph optimization",
  "clean up notes", "link notes".
---
...
> `jfox add` / `edit` / `delete` / `list` / `show` / `daily` 等通用命令以及 daemon 用法详见 `/skill:jfox-common` §4 与 §6–§7。
