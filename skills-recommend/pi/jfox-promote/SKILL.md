...
name: jfox-promote
description: |
  Review/promote gem-synth candidate notes into permanent notes, or reject/archive
  inaccurate ones. 过审 L5 候选宝石，支持大积压的三模式过审（客观去重扫描 /
  簇级 triage / 单条 A/B/C）+ 冗余维度 + 固化机械清理。
  Triggers on: "过审 candidate", "过审宝石", "晋升候选笔记", "审阅候选宝石",
  "candidate 过审", "L5 晋升", "promote candidate", "review candidate",
  "broken candidate", "批量过审", "簇级去重", "dedup 扫描", "candidate 冗余".
---
...
```bash
jfox candidates list --status pending --json | jq '.candidates | length'   # 分页内（≤50；返回 50 = 大积压）
ls "$(jfox kb current --json | jq -r .path)/notes/candidate/" | wc -l  # 文件总数（含 rejected 软删除；纯 pending 看上行 jq）
```
...
try:
    kb_info = json.loads(subprocess.check_output(
        ["jfox", "kb", "current", "--json"], text=True))
...
1. **查是否已被现有 permanent 覆盖**：
   ...
   jfox suggest-links "<簇代表正文>" --json   # 阈值可放宽到 0.4–0.5
   ```
