---
name: jfox-release-cc-plugin
description: |
  Release a new version of the cc-plugin (Claude Code marketplace). Bumps the three
  version fields in lockstep, opens a PR.
  Triggers on: "发 cc-plugin", "plugin 发版", "release cc plugin",
  "bump plugin version", "发布 cc 插件", "cc-plugin release".
---
...
⚠️ 这是 **cc-plugin** 发版（marketplace，0.5.x）。CLI/PyPI 发版（1.x.x）用 `/skill:jfox-release`。
kimi-plugin 是另一条轨道，本 skill 不管。

## 用法

```
/skill:jfox-release-cc-plugin patch     # bump patch: 0.5.1 → 0.5.2
/skill:jfox-release-cc-plugin minor     # bump minor: 0.5.1 → 0.6.0
/skill:jfox-release-cc-plugin major     # bump major: 0.5.1 → 1.0.0
/skill:jfox-release-cc-plugin 0.7.0     # 指定版本
```
...
- 不要用 `/skill:jfox-release`（那是 CLI/PyPI）。
