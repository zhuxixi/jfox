---
name: jfox-release-kimi-plugin
description: |
  Release a new version of the kimi-plugin (Kimi Code 集成). Bumps the single version
  field in kimi.plugin.json, opens a PR.
  Triggers on: "发 kimi-plugin", "release kimi plugin", "bump kimi version",
  "发布 kimi 插件", "kimi-plugin release".
---
...
⚠️ 这是 **kimi-plugin** 发版（0.14.x）。CLI/PyPI 发版（1.x.x）用 `/skill:jfox-release`；cc-plugin 用 `/skill:jfox-release-cc-plugin`。
要三件套一起发，用 `/skill:jfox-release-all`（自动跳过无改动者）。

## 用法

```
/skill:jfox-release-kimi-plugin patch    # 0.14.0 → 0.14.1
/skill:jfox-release-kimi-plugin minor    # 0.14.0 → 0.15.0
/skill:jfox-release-kimi-plugin major    # 0.14.0 → 1.0.0
/skill:jfox-release-kimi-plugin 0.15.0   # 指定版本
```
...
- 三轨道之一；与 `/skill:jfox-release`（CLI/PyPI）、`/skill:jfox-release-cc-plugin`（cc marketplace）并列。
