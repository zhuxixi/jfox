...
- 有信号 → 用 `question` 追问「挂到哪个 MOC？」：选项 = 每个信号 MOC（`label` 用标题，`description` 带 ID 和 score）+「不挂」。手动指定路径保留：用户通过「其他」输入 MOC ID 后，先用 `jfox show <MOC_ID> --json` 核对是未归档的 structure，无效就重新询问或选「不挂」。
- 一条笔记可挂 0 个、1 个或多个 MOC（MOC 成员天然多对多）。`question` 没有多选参数，用顺序追问：先选一个 MOC 或「不挂」；选了 MOC 就再问「继续挂到其他 MOC 吗？」，直到用户选完。
- 每条新笔记独立确认，允许不同笔记挂不同 MOC；不要把一次选择套用到整批。
...
- 多个 MOC 就逐个调用；目标库非默认时，与 `jfox add` 传同一个 `--kb`。
