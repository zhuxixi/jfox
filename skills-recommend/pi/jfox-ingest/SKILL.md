...
description: |
  ...
  Use when user wants to ingest a repository, import git log, import GitHub PRs,
  import GitHub Issues, or bulk import project information.
  Triggers on: "导入仓库", "导入 git log", "导入 PR", "导入 issues",
  "读一下这个仓库", "抓取仓库信息", "ingest repo", "import notes from repository",
  "bulk import from git", "导入项目信息", "ingest repository", "import git history".
---
...
1. 知识库已存在：
   ...
   jfox kb list --json
   ```
   如果没有知识库，提示用户先调用 common 技能（`/skill:jfox-common`）创建。
...
如果用户直接粘贴文本（未提供仓库路径），将文本整理为单条 fleeting 笔记并导入。导入仍遵循下文「笔记格式规范」中的 `source:*` 标签约定；`jfox add` 完整语法与 `--kb` / `--json` / `--content-file` 共享约定详见 `/skill:jfox-common` §4。
...
> 单条笔记命令（`jfox add` / `edit` / `delete` / `show` 等）的完整语法详见 `/skill:jfox-common` §4，共享约定见 §4.1。
...
- **"Knowledge base not found"**: 提示用户先调用 common 技能（`/skill:jfox-common`）创建知识库
