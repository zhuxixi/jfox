...
def assert_note_created(result, title: str = None) -> Dict[str, Any]:
    ...
    if title:
        ...
        ), f"Title mismatch: {note_data.get('title')} != {title}"
...
def assert_link_exists(note_a_id: str, note_b_id: str, cli) -> None:
    ...
    link_ids = [link["id"] for link in forward_links]
...
def assert_backlink_exists(target_id: str, source_id: str, cli) -> None:
    ...
    backlink_ids = [link["id"] for link in backward_links]
...
class TestProcessPhase:
    ...
    def test_process_note_type_conversion(self, cli):
        ...
        permanent_result = cli.add(converted_content, title=converted_title, note_type="permanent")
...
class TestConnectPhase:
    ...
    def test_bidirectional_links(self, cli):
        ...
        note_b_content = "This is note B referencing [[Note A]]"
    ...
    def test_graph_build_after_links(self, cli):
        ...
        create_note_with_type(cli, "Graph Node A", "Node A content", "permanent")

        note_b_content = "Node B content linking to [[Graph Node A]]"
        create_note_with_type(cli, "Graph Node B", note_b_content, "permanent")

        note_c_content = "Node C content linking to [[Graph Node B]]"
        create_note_with_type(cli, "Graph Node C", note_c_content, "permanent")
    ...
    def test_orphan_notes_detection(self, cli):
        ...
        orphan_notes = []
        for i in range(3):
            note = create_note_with_type(
                cli, f"Orphan Note {i}", f"Orphan content {i}", "permanent"
            )
            orphan_notes.append(note)

        # 2. Create linked notes (non-orphan)
        linked_note_a = create_note_with_type(
            cli, "Linked Note A", "Linked note A content", "permanent"
        )
        linked_note_b_content = "Linked note B referencing [[Linked Note A]]"
        linked_note_b = create_note_with_type(
            cli, "Linked Note B", linked_note_b_content, "permanent"
        )
...
class TestDevelopPhaseEmbedding:
    ...
    def test_semantic_search_discover(self, cli, generator):
        ...
        assert any(
            "Python" in t for t in top_titles
        ), "Python-related note should be in top results"
    ...
    def test_suggest_links_semantic(self, cli, generator):
        ...
        new_content = (
            "I'm learning about neural networks and how to train them using backpropagation"
        )
        ...
        has_ml_suggestion = any(any(kw in t for kw in ml_keywords) for t in suggested_titles)
    ...
    def test_multi_kb_isolation(self, cli):
        ...
        with TemporaryKnowledgeBase() as kb1_path:
            with TemporaryKnowledgeBase() as kb2_path:
                ...
                create_note_with_type(cli1, "KB1 Note", "Only in KB1", "permanent")

                # 3. Create note in kb2
                create_note_with_type(cli2, "KB2 Note", "Only in KB2", "permanent")
...
class TestCompleteWorkflow:
    ...
    def test_capture_to_develop_full_workflow(self, cli):
        ...
        linked_note = create_note_with_type(
            cli, "Async Best Practices", linked_note_content, "permanent"
        )
