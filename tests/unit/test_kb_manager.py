...
from jfox.global_config import DEFAULT_KB_PATH, KnowledgeBaseEntry
from jfox.kb_manager import (
    KBStats,
...
class TestKnowledgeBaseManager:
    ...
    @patch("jfox.kb_manager.get_global_config_manager")
    ...
    def test_create_success(self, manager, mock_config_manager):
        ...
        with patch("jfox.kb_manager.ZKConfig") as mock_zk_config:
    ...
    def test_create_path_already_used(self, manager, mock_config_manager):
        ...
        existing_entry = KnowledgeBaseEntry(
            name="other_kb", path=str(existing_path), created="2024-01-01T00:00:00"
        )
        mock_config_manager.list_knowledge_bases.return_value = [existing_entry]

        success, message = manager.create(name="new_kb", path=existing_path)
    ...
    def test_create_with_explicit_path(self, manager, mock_config_manager):
        ...
        with patch("jfox.kb_manager.ZKConfig") as mock_zk_config:
            ...
            success, message = manager.create(name="custom_kb", path=DEFAULT_KB_PATH / "custom")
    ...
    def test_remove_delete_data_fails(self, manager, mock_config_manager, tmp_path):
        ...
        # 模拟 shutil.rmtree 失败
        with patch("jfox.kb_manager.shutil.rmtree", side_effect=PermissionError("Access denied")):
            success, message = manager.remove(name="test_kb", delete_data=True)

        assert success is True  # 移除配置仍然成功
        assert "failed to delete data" in message

    def test_remove_default_kb_with_delete_data_protected(
        self, manager, mock_config_manager, tmp_path
    ):
    ...
    def test_list_all_with_entries(self, manager, mock_config_manager, tmp_path):
        ...
        entry = KnowledgeBaseEntry(
            name="test_kb", path=str(tmp_path), created="2024-01-01T00:00:00"
        )
    ...
    def test_get_kb_stats_nonexistent_path(self, manager):
        """测试统计不存在路径的知识库"""
        entry = KnowledgeBaseEntry(
            name="test_kb", path="/nonexistent/path", created="2024-01-01T00:00:00"
        )
    ...
    def test_get_current_kb_info(self, manager, mock_config_manager, tmp_path):
        """测试获取当前知识库信息"""
        entry = KnowledgeBaseEntry(
            name="current_kb", path=str(tmp_path), created="2024-01-01T00:00:00"
        )
...
class TestGetKbManager:
    ...
    def setup_method(self):
        """每个测试前清理全局实例"""
        import jfox.kb_manager as kb_module
