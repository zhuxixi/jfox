...
class TestBacklinksAutoMaintenance:
    ...
    def test_create_note_updates_target_backlinks(self, cli):
        ...
        # 1. 创建目标笔记
        target_result = cli.add("Target note content", title="Target Note", note_type="permanent")
        assert target_result.success
        target_id = target_result.data["note"]["id"]

        # 2. 创建源笔记，链接到目标
        source_result = cli.add(
            "Source note referencing [[Target Note]]", title="Source Note", note_type="permanent"
        )
    ...
    def test_backlink_not_duplicated(self, cli):
        ...
        # 2. 创建源笔记链接到目标
        source = cli.add(
            "Source referencing [[No Duplicate Target]]", title="Source Note", note_type="permanent"
        )
        assert source.success
        source.data["note"]["id"]
    ...
    def test_chained_backlinks(self, cli):
        ...
        # B -> A
        note_b = cli.add("Note B referencing [[Note A]]", title="Note B", note_type="permanent")
        assert note_b.success
        id_b = note_b.data["note"]["id"]

        # C -> B
        note_c = cli.add("Note C referencing [[Note B]]", title="Note C", note_type="permanent")
        assert note_c.success
        id_c = note_c.data["note"]["id"]

        # 验证 A 有来自 B 的反向链接
        refs_a = cli.refs(note_id=id_a)
        backlink_ids_a = [link["id"] for link in refs_a.data.get("backward_links", [])]
        assert id_b in backlink_ids_a

        # 验证 B 有来自 C 的反向链接
        refs_b = cli.refs(note_id=id_b)
        backlink_ids_b = [link["id"] for link in refs_b.data.get("backward_links", [])]
