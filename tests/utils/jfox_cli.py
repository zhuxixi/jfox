...
class ZKCLI:
    ...
    def _run(
        self, cmd: str, *args, capture_output: bool = True, json_output: bool = True
    ) -> CLIResult:
        """
        运行 CLI 命令
    ...
    def refs(self, note_id: Optional[str] = None, search: Optional[str] = None) -> CLIResult:
        """查看引用关系"""
        args = []
        if note_id:
    ...
    def kb_create(
        self, name: str, path: Optional[Path] = None, description: Optional[str] = None
    ) -> CLIResult:
        """创建知识库"""
        args = ["create", name]
        if path:
    ...
    def query(self, query_str: str, top: int = 5, graph_depth: int = 2) -> CLIResult:
        """
        语义搜索 + 知识图谱联合查询
    ...
    def suggest_links(self, content: str, top_k: int = 5, threshold: float = 0.6) -> CLIResult:
        """
        根据内容推荐可以链接的已有笔记
