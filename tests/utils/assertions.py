...
class ZKAssertions:
    ...
    def assert_success(result, message: Optional[str] = None):
        ...
        assert result.success, (
            message or f"Command failed with returncode {result.returncode}: {result.stderr}"
        )
    ...
    def assert_failure(result, message: Optional[str] = None):
        ...
        assert not result.success, (
            message or f"Command should have failed but succeeded: {result.stdout}"
        )
    ...
    def assert_note_count(cli, expected_count: int, note_type: Optional[str] = None):
        ...
        ), f"Expected {expected_count} notes, got {actual_count}"
    ...
    def assert_search_finds(cli, query: str, expected_note_id: str, top: int = 10):
        ...
        assert (
            expected_note_id in found_ids
        ), f"Note {expected_note_id} not found in search results for '{query}'"
    ...
    def assert_valid_note_data(note_data: Dict[str, Any], expected_id: Optional[str] = None):
        ...
        if expected_id:
            assert (
                note_data["id"] == expected_id
            ), f"Note ID mismatch: expected {expected_id}, got {note_data['id']}"
