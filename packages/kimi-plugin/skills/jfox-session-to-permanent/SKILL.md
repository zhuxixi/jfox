...
一句话：jfox-session-summary 记「我最近做了哪些动作」（历史），本技能沉淀「事情当前是什么状态」（现状），jfox-organize 整理「早就堆在收件箱的笔记」。

## 前置条件

- 知识库已初始化（`jfox init`）。未初始化时，提示用户先调用 `/skill:jfox-manage` 创建。
...
**严禁未经审阅直接 `jfox add`。** 把草稿完整展示给用户，等明确确认。这是本技能和 `jfox-session-summary`（直接写入）最大的区别——session 笔记是存档可以存完再改，permanent 是知识沉淀，落库前必须过眼。确认环节一律用 AskUserQuestion 出选择题，用户方向键选择、回车即确认，不用打字回复。
...
本技能专属命令（通用命令见 `/skill:jfox-manage` §4）：
...
- **`jfox search` / `suggest-links` 因 daemon 未启动而失败** → 提示用户启动 daemon（见 `/skill:jfox-manage` §6），或先用 `jfox search` 关键词模式（不依赖 embedding）。
...
- **知识库指定**：用 `--kb <name>` 指定目标知识库（约定见 `/skill:jfox-manage` §4.1），省略时写入当前默认知识库。
